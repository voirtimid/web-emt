...
public class JobService {
    ...
    public JobId createJob(JobForm jobForm) {
        ...
        job.getJobEntries().forEach(jobEntry -> {
            applicationEventPublisher.publishEvent(new JobEntryAdded(Instant.now(), job.getId(), jobEntry.getId(), job.getEmployeeId(), jobEntry.getTaskId()));
        });
    ...
    private Job toDomainModel(@NonNull JobForm jobForm) {
        Job job = new Job(jobForm.getEmployeeId(), Instant.now());
